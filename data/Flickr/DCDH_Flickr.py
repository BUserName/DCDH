...
parser.add_argument('--max-iter', default=121, type=int,
...
def _dataset():
    ...
    dset_database = dp.DatasetProcessingNUS_WIDE(
        'data/Flickr', 'db_image.txt', 'database_label.txt', transformations
    )
    dset_test = dp.DatasetProcessingNUS_WIDE(
        'data/Flickr', 'test_image_m.txt', 'test_label.txt', transformations
    )
    ...
    databaselabels = load_label('database_label.txt', 'data/Flickr')
    testlabels = load_label('test_label.txt', 'data/Flickr')
